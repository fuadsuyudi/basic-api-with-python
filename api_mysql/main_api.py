...
def list_people():
    ...
    try:
        for row in row_data:
            data.append({'first_name':row[1],'last_name':row[2],'email':str(row[3])})
...
def add_people():
    try:
        dbf.add_people(request.query.first_name, request.query.last_name, request.query.email)
...
def get_people(name):
    ...
    try:
        for row in row_data:
            data.append({'first_name':row[1],'last_name':row[2],'email':row[3]})
